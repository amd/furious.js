...
JSContext.prototype.min = function(inArray, axis) {
	if (typeof axis === 'undefined') {
		...
		var outerStride = util.getAxisReductionOuterStride(outShape, axis);
		var innerStride = util.getAxisReductionInnerStride(outShape, axis);
	...
	if (typeof axis === 'undefined') {
		...
		for (var i = 0; i < outerStride; ++i) {
			for (var k = 0; k < innerStride; ++k) {
				...
				for (var j = 1; j < reductionDim; ++j) {
					offset += innerStride;
					currentMin = Math.max(currentMin, inArray.data[offset]);
				}
				outArray.data[i * innerStride + k] = currentMin;
			}
		}
	...
	if (typeof axis === 'undefined') {
		...
		for (var i = 0; i < outerStride; ++i) {
			for (var k = 0; k < innerStride; ++k) {
				...
				for (var j = 1; j < reductionDim; ++j) {
					offset += innerStride;
					currentMin += inArray.data[offset];
				}
				outArray.data[i * innerStride + k] = currentMin;
			}
		}
