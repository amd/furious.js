...
JSContext.prototype.toArray = function(array, callback) {
	if (!(array instanceof NDArray)) {
		throw new TypeError("NDArray object expected");
	}
	if (array.shape.length === 0) {
		callback(array.data[0]);
	} else {
		var jsarray = new Array(array.shape[0]);
		util.createArrayRecursive(array.data, jsarray, array.shape, 0, 0);
		callback(jsarray);
	}
};
		...
		for (var j = 0; j < reductionLength; j++) {
			for (var k = 0; k < bPreStride; k++) {
				for (var l = 0; l < bPostStride; l++) {
					output.data[(i*bPreStride + k) * bPostStride + l] += a.data[i*reductionLength+j] * b.data[(k*reductionLength+j)*bPostStride+l];
				}
			}
		}
	}
