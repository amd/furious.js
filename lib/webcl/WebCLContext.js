...
var isFP64Capable = function(device) {
	/* Work-around for Nokia WebCL, which exposes OpenCL extensions directly, but does not list any WebCL extensions */
	var extensionString = device.getInfo(cl.DEVICE_EXTENSIONS);
	if (/\bcl_khr_fp64\b/.exec(extensionString)) {
		return true;
	}
	/* Normal check for WebCL KHR_fp64 extension */
