...
WebCLContext.prototype.empty = function(shape, dataType) {
	...
	array._buffer = context.createBuffer(webcl.MEM_READ_WRITE, array.length * dataType.size);
	return array;
};
...
WebCLContext.prototype.array = function(data, dataType) {
	...
	if (useBufferCreationWithInit) {
		array._buffer = context.createBuffer(webcl.MEM_READ_WRITE, buffer.byteLength, buffer);
	} else {
		array._buffer = context.createBuffer(webcl.MEM_READ_WRITE, buffer.byteLength);
	...
	if (useAsyncBufferRead) {
		...
		for (var i = 0; i < callbackWaitArguments; i++) {
			...
			(function(i, shape, ArrayType) {
				...
				if (shape.length === 0) {
					readFinishEvent.setCallback(webcl.COMPLETE, function() {
