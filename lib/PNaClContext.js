...
function PNaClContext(callback) {
	var context = this;
	this.pnacl = document.createElement("object");
	this.pnacl.width = 0;
	this.pnacl.height = 0;
	this.pnacl.data = "furious.nmf";
	this.pnacl.type = "application/x-pnacl";
	this.pnacl.addEventListener("load", function () {
		var messageId = allocator.newMessageId();
		messageCallbacks[messageId] = function() {
			callback(context);
		};
		context.pnacl.postMessage({
			"id": messageId,
			"command": "init"
		});
	}, true);
	this.pnacl.addEventListener("message", onPNaClMessage, true);
	document.body.appendChild(this.pnacl);
}
...
PNaClContext.prototype.get = function(array, callback) {
