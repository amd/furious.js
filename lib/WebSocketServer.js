...
ConnectionContext.prototype.binaryOperation = function(requestId, type, idA, idB, idOut) {
    ...
    var shapeOut = arrayA.shape;
    var dataTypeOut = arrayA.dataType;
    if (type === 0) {
        var kernel = this.kernels.add[arrayA.dataType.type];
        ...
        this.queue.enqueueNDRangeKernel(kernel, 1, null, [arrayA.length], null);
        var arrayOut = {
            buffer: bufferOut,
            shape: shapeOut,
            length: util.computeLength(shapeOut),
