...
function createDataArray(requestId, idOut, shape, dataType, dataBuffer, connectionContext) {
    if (idOut in connectionContext.idMap) {
        console.error("Error: invalid output ID in createDataArray")
        return;
    }
    dataType = util.checkDataType(dataType);
    var buffer = connectionContext.context.createBuffer(nodeWebCL.MEM_READ_WRITE, dataBuffer.byteLength);
    // TODO: use non-blocking write
    connectionContext.queue.enqueueWriteBuffer(buffer, true, 0, dataBuffer.byteLength, dataBuffer);
    var arrayOut = {
        buffer: buffer,
        shape: shape,
    ...
    connectionContext.idMap[idOut] = arrayOut;
}

function deallocate(requestId, idA, connectionContext) {
    var arrayA = connectionContext.idMap[idA];
    if (typeof arrayA === "undefined") {
        console.error("Error: invalid input ID in deallocate");
        return;
    }
    arrayA.buffer.release();
    delete connectionContext.idMap[idA];
}

function fetch(requestId, idA, connectionContext) {
    var arrayA = connectionContext.idMap[Math.abs(idA)];
    if (typeof arrayA === "undefined") {
    ...
    connectionContext.queue.enqueueReadBuffer(arrayA.buffer, true, 0, dataBuffer.byteLength, dataBuffer);
...
server.on("connection", function(connection) {
    connection.connectionContext = new ConnectionContext({id: count});
