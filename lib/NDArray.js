...
(function(exports){
	...
	function maxReduceRecursive(data, currentMax, shape, level, offset) {
		var n = shape[level];
		if (level === shape.length - 1) {
			for (var i = 0; i < n; i++) {
				var idx = offset * n + i;
				currentMax = Math.max(currentMax, data[idx]);
			}
		} else {
			for (var i = 0; i < n; i++) {
				currentMax = maxReduceRecursive(data, currentMax, shape, level + 1, offset * n + i);
			}
		}
		return currentMax;
	}

	function sumReduceRecursive(data, currentSum, shape, level, offset) {
		var n = shape[level];
		if (level === shape.length - 1) {
			for (var i = 0; i < n; i++) {
				var idx = offset * n + i;
			...
			for (var i = 0; i < n; i++) {
				currentSum = sumReduceRecursive(data, currentSum, shape, level + 1, offset * n + i);
			}
		}
		return currentSum;
	}
	...
	NDArray.prototype.min = function(axis) {
		if (typeof axis === 'undefined') {
			return minReduceRecursive(this.data, Number.POSITIVE_INFINITY, this.shape, 0, 0);
	...
	NDArray.prototype.max = function(axis) {
		if (typeof axis === 'undefined') {
			return maxReduceRecursive(this.data, Number.NEGATIVE_INFINITY, this.shape, 0, 0);
		} else if (util.isInt(axis)) {
			if ((axis < 0) || (axis >= this.shape.length)) {
				throw new RangeError("Invalid axis value " + axis);
			}
	...
	NDArray.prototype.sum = function(axis) {
		if (typeof axis === 'undefined') {
			return sumReduceRecursive(this.data, 0.0, this.shape, 0, 0);
		} else if (util.isInt(axis)) {
			if ((axis < 0) || (axis >= this.shape.length)) {
				throw new RangeError("Invalid axis value " + axis);
			}
	...
	function abs(input, output) {
		...
		function absRecursive(inData, outData, shape, level, offset) {
			var n = shape[level];
			if (level === shape.length - 1) {
				for (var i = 0; i < n; i++) {
					var idx = offset * n + i;
					outData[idx] = Math.abs(inData[idx]);
				}
			} else {
				for (var i = 0; i < n; i++) {
					absRecursive(inData, outData, shape, level + 1, offset * n + i);
				}
			}
		}
		absRecursive(input.data, output.data, input.shape, 0, 0);
		return output;
	}
	...
	function log(input, output) {
		...
		function logRecursive(inData, outData, shape, level, offset) {
			var n = shape[level];
			if (level === shape.length - 1) {
				for (var i = 0; i < n; i++) {
					var idx = offset * n + i;
					outData[idx] = Math.log(inData[idx]);
				}
			} else {
				for (var i = 0; i < n; i++) {
					logRecursive(inData, outData, shape, level + 1, offset * n + i);
				}
			}
		}
		logRecursive(input.data, output.data, input.shape, 0, 0);
		return output;
	}
